...
#        self._path_to_compiled_app = None # e.g. onfig["ROOT_SCRIPTS_BINS"], "get_calibration_function")
    ...
    def rebin(self, nbins=10):
      ...
      ##
      ## Seba 18 March 2025: need to recompute yerr?! So far only meaningful for rate-vs-time plots w/o rebinning
    ...
    def load_spectrum(self, thrl=10, thrh=3100, preview=False):
        ...
        if not thrl is None or not thrh is None:
          xlim = (bc>thrl) & (bc<thrh)
        else:
          xlim = np.array( len(bc)*[True] )

        self.x = bc[xlim]
        self.y = h_cont[xlim]
        self.yerr = hist.errors()[xlim]
    ...
    def init_manual_dict(self): # manually added isotopes with parameters
        self.manual_dict = {
#               'Pb214_old': [( 351.932, 37.6), (295.224, 19.3), (241.997, 7.43), 
...
#                         ],
               'Pb214': [( 351.932, 37.6), (295.224, 19.3), (241.997, 7.43),
                          (785.96, 1.07), (839.04, 0.587),
                         (258.87, 0.524), (274.80, 0.474), (487.09, 0.422),
                         (580.13, 0.352),
                         (480.43, 0.320), (462.00, 0.221),
                         (533.66, 0.186),
                         (77.107, 8.6),
                         (74.815, 5.13),
                         (87.349, 1.98),
                         ],
#               'Bi214_old': [(609.312, 46.1),
#                         (665.453, 1.46),
#                         (768.356, 4.94),
#                         (934.061, 3.03),
#                         (1120.287, 15.1),
...
#               'Pb212_old' : [(238.632, 43.3), (300.087, 3.28), (115.183, 0.592), (415.2, 0.143), (176.68, 0.052)],
               'Pb212' : [(238.632, 43.3), (300.087, 3.28), (115.183, 0.592),(74.815, 9.9),
                          (77.107, 16.4),
                          (87.349, 3.81),
                          (89.784, 1.40), (415.2, 0.143), (176.68, 0.052)],
               'Bi212' : [(727.330, 6.58),
                         (785.37, 1.102),
                         (1620.50, 1.49),
               ...
               'Tl208' : [#(10.6, 2.75), ## too low energy, causes problems when trying to plot outside range of histogram.
                          (72.805, 2.01), (74.969, 3.35), (84.45, 0.404), (84.938, 0.776), (87.3, 0.283),
                          (211.40, 0.180), (233.36, 0.310), (252.61, 0.780), (277.371, 6.6), (485.95, 0.049),
                          (510.77, 22.60), (583.187, 85.0),
                          #(510.77, 22.0),
                          #(583.191, 83.2),
                          (860.564, 12.5),
